...
// nfa constructs a finite automaton for recognising a single character.
...
func (c *characterSet) match(char rune) bool {
    for _, cs := range c.charSets {
        if (c.exclude && !cs.match(char)) ||
            (!c.exclude && cs.match(char)) {
            return true
        }
    }
    return false
